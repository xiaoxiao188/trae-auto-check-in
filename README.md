...
### 明文变量（`wrangler.toml` 的 `[vars]`，随部署上传）

| 变量 | 说明 |
|---|---|
| `CRON_LABEL` | 推送消息中显示的定时说明，如 `每天 08:30（北京时间）` |

⚠️ **不要把密钥写进 `wrangler.toml` 的 `[vars]`**，那里是明文的，会泄露账号凭证。
...
[vars]
CRON_LABEL = "每天 08:30（北京时间）"   # 同步修改说明文字
```
